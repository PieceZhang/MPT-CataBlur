...
class CKPT_Manager:
    def __init__(self, root_dir, model_name, cuda, max_files_to_keep = 10, is_descending = False):
        ...
        self.ckptlog_list = os.path.join(self.root_dir, 'ckptlogs.txt')
    ...
    def load_ckpt(self, network, by_score = True, name = None, abs_name = None, epoch = None):
        ...
        if self.cuda is False:
            ...
            for k, v in state_dict.items():
                k = k.split('.', 1)[-1]
                new_state_dict[k] = v
            f = lambda x: 'Network.' + x
            new_state_dict = {f(key): value for key, value in new_state_dict.items()}
            return network.load_state_dict(new_state_dict, strict=False), os.path.basename(file_name)

        else:
            device_id = torch.cuda.current_device()
            state_dict = torch.load(file_path, map_location="cuda:{}".format(device_id) if self.cuda else "cpu")
            try:
                return network.load_state_dict(state_dict, strict=True), os.path.basename(file_name)
            except:
                # state_dict = state_dict['params'] if 'params' in state_dict else state_dict
                f = lambda x: 'module.' + x
    ...
    def save(self, network, state, epoch, score):
        ...
        with open(self.ckptlog_list, 'a') as file:
            #line_to_add = file_name + ' ' + str(score) + os.linesep
            line_to_add = file_name
            for s in score:
                line_to_add = line_to_add + ' ' + str(s)
